...
//    doc.collection.url = '/collection/';
  ...
  QUnit.test('has', function(assert) {
    ...
    // 可以set 属性值 == null的属性,只不过has判定模型没有该属性
    model.set({
      '0': 0,
      '1': 1,
      'true': true,
  ...
  QUnit.test('multiple unsets', function(assert) {
    ...
    model.set({a: 2});
    model.unset('a');
    // unset一个不存在的属性时,#499的两个equal都是true(val为undefined,
    // 因为调用unset时不会传入第二个参数(val)),因而changes和changed
    // 都没有记下这个属性,当然也就不会触发事件了
    model.unset('a');
  ...
  QUnit.test('setting an object', function(assert) {
    ...
    // set方法里用到的_.isEqual是'deep equal'的比较,可以处理对象
    model.set({
  ...
  QUnit.test('save within change event', function(assert) {
    ...
    model.on('change', function() {
      model.save();
      // setup/environment.js中使用了env.syncArgs捕获sync的参数
